...
public class CalculateTimeUtils {
     ...
     */
    public static BigDecimal CalculateHourSubtractHolidayHour(String beginTime, String endTime, BigDecimal allHours) throws ParseException {
        CalculateTimeUtils ch = new CalculateTimeUtils();
        Calendar startDay = Calendar.getInstance();//获取系统当前时间  开始时间
        Calendar endDay = Calendar.getInstance();//获取系统当前时间 结束时间
        ...
        int holidays = arrContrast(workday, arr);//两个日期之间有多少个节假日 不包括首尾两个日期，剩下的就是判断首尾是否是节假日(不用判断周末)
        BigDecimal holidaysHours = new BigDecimal("7.5").multiply(new BigDecimal(String.valueOf(holidays)));//两个日期的小时数
        //判断首日是否是节假日
        List<String> holidayList = Arrays.asList(arr);
        BigDecimal subtractStartDayHour = BigDecimal.ZERO;
        BigDecimal subtractEndDayHour = BigDecimal.ZERO;
        String beginDateDayStr = DateUtil.format(ch.stringToDate(beginTime), NORM_DATE_FORMAT);
        String endDateDayStr = DateUtil.format(ch.stringToDate(endTime), NORM_DATE_FORMAT);
        if (holidayList.contains(beginDateDayStr) && holidayList.contains(endDateDayStr)) {
            return BigDecimal.ZERO;
        }
        if (holidayList.contains(beginDateDayStr)) {
            //如果首日是节假日，则算出该日 工时
            String beginTime4EndTime = ch.getEveningDateString(ch.stringToDate(beginTime));//开始时间
            subtractStartDayHour = new BigDecimal(Float.toString(ch.calculateHours(beginTime, beginTime4EndTime)));
            if (subtractStartDayHour.compareTo(BigDecimal.ZERO) <= 0) {
                subtractStartDayHour = BigDecimal.ZERO;
            }
        }
        if (holidayList.contains(endDateDayStr)) {
            //如果结束日是节假日，则算出该日 工时
            String endTime4StartTime = ch.getMorningDateString(ch.stringToDate(endTime));//先获取结束日 计算的开始时间
            subtractEndDayHour = new BigDecimal(Float.toString(ch.calculateHours(endTime4StartTime, endTime)));
            if (subtractEndDayHour.compareTo(BigDecimal.ZERO) <= 0) {
                subtractEndDayHour = BigDecimal.ZERO;
            }
        }
        BigDecimal resultHour = allHours.subtract(holidaysHours).subtract(subtractStartDayHour).subtract(subtractEndDayHour);
        return resultHour;
    }

    public static String getMorningDateString(Date date) {
        Date nowZeroDate = DateUtil.beginOfDay(date);
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(nowZeroDate);
        calendar.add(Calendar.HOUR, 9);
        nowZeroDate = calendar.getTime();
        return DateUtil.format(nowZeroDate, NORM_DATETIME_FORMAT);
    }

    public static String getEveningDateString(Date date) {
        Date nowZeroDate = DateUtil.beginOfDay(date);
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(nowZeroDate);
    ...
    //9:00 -12:00 3h   13:30 -18:00  4.5     all 7.5
    public static void main(String[] args) throws ParseException {
        String beginTime = "2023-05-03 19:20:00";//6.166667
        String endTime = "2023-05-04 10:00:00";//4
        CalculateTimeUtils ch = new CalculateTimeUtils();
        float b = ch.calculateHours(beginTime, endTime);
        System.out.println(b);
        BigDecimal allHours = new BigDecimal(Float.toString(b));
        //float a = CalculateTimeUtils.CalculateHour(beginTime, endTime);
        //System.out.println(a);
        BigDecimal resultHours = CalculateTimeUtils.CalculateHourSubtractHolidayHour(beginTime, endTime, allHours);
